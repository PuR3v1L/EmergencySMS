...
public class SeekBarPreference extends Preference implements OnSeekBarChangeListener {
	...
	private static int mMaxValue = 15;
	private static int mMinValue = 5;
	...
	private void initPreference(Context context, AttributeSet attrs) {
		instantSMSemergensy = (InstantSMSemergensy) context.getApplicationContext();
		...
		mSeekBar.setOnSeekBarChangeListener(this);
			...
			setmUnitsRight(" " + instantSMSemergensy.getMyResources().getString(R.string.times));
...
//		Log.d(TAG,"first newValue: "+newValue);

		if (newValue > mMaxValue)
			newValue = mMaxValue;
		else if (newValue < mMinValue)
			newValue = mMinValue;
		if (mInterval != 1 && newValue % mInterval != 1)
			newValue = Math.round(((float) newValue) / mInterval) * mInterval -1;
